...
public abstract class AbstractChannel extends DefaultAttributeMap implements Channel {
    ...
    protected AbstractChannel(Channel parent) {
        ...
        // 每个channel拥有的一个唯一id
        id = newId();
        // 客户端：类型是 AbstractNioByteChannel.NioByteUnsafe 内部类
        // 服务端：类型是 AbstractNioMessageChannel.NioMessageUnsafe 内部类
        // Unsafe 对象是 Java 底层的 Socket 操作的封装
        unsafe = newUnsafe();
        // 创建当前channel对应的pipeline
    ...
    protected abstract class AbstractUnsafe implements Unsafe {
        ...
        public final void register(EventLoop eventLoop, final ChannelPromise promise) {
            ...
            //是完成 EventLoop 与 Channel 的关联过程，后续IO请求由该EventLoop处理
            AbstractChannel.this.eventLoop = eventLoop;

            //判断EventLoop中的线程是否当前线程
            if (eventLoop.inEventLoop()) {
                //真正注册的方法
                register0(promise);
            } else {
                try {
                    //启动EventLoop的本地线程执行任务
        ...
        private void register0(ChannelPromise promise) {
            try {
                ...
                //AbstractNioChannel调用java原生API注册
                doRegister();
