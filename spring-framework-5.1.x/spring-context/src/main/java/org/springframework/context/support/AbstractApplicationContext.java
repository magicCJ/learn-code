 ...
 *
 * <p>与普通的BeanFactory相比，应假定ApplicationContext 用于检测在其内部bean工厂中定义的特殊bean：因此，此类自动注册
 * {@link org.springframework.beans.factory.config.BeanFactoryPostProcessor BeanFactoryPostProcessors},
 * {@link org.springframework.beans.factory.config.BeanPostProcessor BeanPostProcessors},
 * and {@link org.springframework.context.ApplicationListener ApplicationListeners}
 * 在上下文中被定义为bean。
 *
 * <p>A {@link org.springframework.context.MessageSource} 也可以作为上下文中的bean提供，
 * 名称为“ messageSource”；否则，将消息解决方案委派给父上下文。此外，可以在上下文中将类型为
 * {@link org.springframework.context.event.ApplicationEventMulticaster}
 * 的“ applicationEventMulticaster” bean 提供用于应用程序事件的多播器；否则，是类型的默认多播器
...
public abstract class AbstractApplicationContext extends DefaultResourceLoader
	...
	public AbstractApplicationContext(@Nullable ApplicationContext parent) {
		//初始化一个资源解析器(含资源加载器)，资源加载器是当前对象本身
		this();
		//获取系统信息
		setParent(parent);
	}
	...
	/**
	 * 准备该上下文以进行刷新，设置其启动日期和active标志以及执行属性源的任何初始化。
	 */
	protected void prepareRefresh() {
		...
		// 允许注册监听早期的ApplicationEvent，一旦多播器可用时就发布...
	...
	/**
	 * 实例化并调用所有已注册的BeanFactoryPostProcessorBean，遵循显式顺序（如果给定的话）。
	 * <p>必须在单例实例化之前调用。
	 */
	...
	/**
	 * 如果实现了 ConfigurableApplicationContext，则返回父类的内部bean工厂。否则，返回父类本身。
	 ...
	 * <p>子类将创建一个新的bean工厂并保留对其的引用，或者返回其持有的单个BeanFactory实例。在后一种情况下，
