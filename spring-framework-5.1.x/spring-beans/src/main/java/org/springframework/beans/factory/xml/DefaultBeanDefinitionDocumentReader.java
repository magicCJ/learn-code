 ...
 * 该接口根据“ spring-beans” DTD和XSD格式（Spring的默认XML bean定义格式）读取bean定义。
 *
 * <p>所需XML文档的结构，元素和属性名称在此类中进行了硬编码。 （当然，如有必要，可以运行转换以产生这种格式）。
 * {@code <beans>}不必是XML文档的root 元素：此类将解析XML文件中的所有bean定义元素，而与实际的root元素无关。
 *
 * @author Rod Johnson
...
public class DefaultBeanDefinitionDocumentReader implements BeanDefinitionDocumentReader {
	...
	protected void doRegisterBeanDefinitions(Element root) {
		// 任何嵌套的<beans>元素都将在此方法中进行递归。为了正确传播和保留<beans> default- *属性，
		// 跟踪当前（父）委托，该委托可以为null。 创建新的（子）委托，并带有对父级的引用，以进行回退，
		// 然后最终将this.delegate重置回其原始（父）引用。此行为模拟了一组委托，而实际上并没有必要。

		// 具体的解析过程由BeanDefinitionParserDelegate实现，其中定义了对Spring Bean定义XML文件的各种元素解析
	...
	protected void parseBeanDefinitions(Element root, BeanDefinitionParserDelegate delegate) {
		//Bean定义的文档对象是否使用了Spring默认的XML命名空间
