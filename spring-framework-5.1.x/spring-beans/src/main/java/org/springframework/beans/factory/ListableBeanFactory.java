 ...
 *
 * <p>如果这是{@link HierarchicalBeanFactory}，则返回值<i>no</ i> 将考虑任何BeanFactory层次结构，
 * 而仅与当前工厂中定义的bean有关。也可以使用{@link BeanFactoryUtils}帮助程序类来考虑父类工厂中的bean。
 *
 ...
 * 注册的任何单例bean，但{@code getBeanNamesOfType }和{@code getBeansOfType}
 * 也会检查这样的手动注册的单例。当然，BeanFactory的{@code getBean} 确实允许透明访问此类特殊bean。
 * 但是在典型的场景中，无论如何，所有bean都将由外部bean定义来定义，因此大多数应用程序不需要担心这种区别。
 *
