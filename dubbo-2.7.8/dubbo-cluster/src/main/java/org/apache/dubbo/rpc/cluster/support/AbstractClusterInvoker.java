...
public abstract class AbstractClusterInvoker<T> implements ClusterInvoker<T> {
    ...
    protected Invoker<T> select(LoadBalance loadbalance, Invocation invocation,
        ...
        // 在 sticky 为 true，且 stickyInvoker != null 的情况下。如果 selected 包含stickyInvoker
        // 说明 stickyInvoker 在此之前没有成功提供服务（但其仍然处于存活状态）。此时我们认为这个服务不可靠，不应该在重试期间内再次被调用
    ...
    private Invoker<T> doSelect(LoadBalance loadbalance, Invocation invocation,
        ...
        // 如果 selected 包含负载均衡选择出的 Invoker 即不稳定，或者该 Invoker 无法经过可用性检查，此时进行重选
