...
public class DubboProtocol extends AbstractProtocol {
    ...
    public <T> Invoker<T> protocolBindingRefer(Class<T> serviceType, URL url) throws RpcException {
        ...
        // create rpc invoker.需要基于更底层的客户端进行网络通信
    ...
    private List<ReferenceCountExchangeClient> getSharedClient(URL url, int connectNum) {
        ...
        synchronized (locks.get(key)) {
            ...
            if (CollectionUtils.isEmpty(clients)) {
                // 创建 ExchangeClient 客户端，并将 ExchangeClient 实例传给 ReferenceCountExchangeClient，这里使用了装饰模式
                ...
                for (int i = 0; i < clients.size(); i++) {
                    ...
                    // 如果列表中有一个不再可用的客户端，则创建一个新客户端以替换该客户端。
    ...
    private ExchangeClient initClient(URL url) {
        ...
        try {
            ...
            if (url.getParameter(LAZY_CONNECT_KEY, false)) {
                ...
                // 在 request 方法被调用时通过 Exchangers 的 connect 方法创建 ExchangeClient 客户端
